...
router.post('/doctor/register',
 ...
 router.get('/google/callback', 
    passport.authenticate('google', {
        session:false,
        failureRedirect: process.env.FRONTEND_URL ? `${process.env.FRONTEND_URL}/auth/error?message=${encodeURIComponent('Google authentication failed')}` : '/auth/failure'
    }),
    ...
    async(req,res) => {
        try {
             if (!req.user) {
                 ...
                 return res.redirect(`${frontendUrl}/auth/error?message=${encodeURIComponent('Authentication failed - user not found')}`);
             }
